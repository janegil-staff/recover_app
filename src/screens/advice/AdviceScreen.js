...
const CATEGORIES = ["all","cravings","mood","sleep","medication","wellbeing","relationships"];
const CAT_ICONS  = { all:"🌟",cravings:"🔥",mood:"😊",sleep:"😴",medication:"💊",wellbeing:"💙",relationships:"🤝" };
const STATUS_COLORS = { new:"#22C55E", viewed:"#9CA3AF", relevant:"#F97316" };
...
export default function AdviceScreen({ navigation }) {
  ...
  const toggleRelevant = useCallback(async (id) => {
    setRelevant(prev => {
      const next = new Set(prev);
  ...
  const catLabel = (c) => t[`adviceCat_${c}`] ?? c.charAt(0).toUpperCase() + c.slice(1);
  ...
  return (
    <SafeAreaView style={{ flex:1, backgroundColor: theme.bg }} edges={["bottom"]}>
      ...
      <View style={s.toggleRow}>
        {["all","relevant"].map(f => (
          <TouchableOpacity key={f} onPress={() => setFilter(f)}
            style={[s.toggleBtn, filter===f && { backgroundColor: theme.accent }]}>
            <Text style={[s.toggleText, filter===f && { color:"#fff" }]}>
              {f==="all" ? (t.adviceAll ?? "ALL") : (t.adviceRelevant ?? "RELEVANT FOR ME")}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}
        style={{ maxHeight: 44 }} contentContainerStyle={s.catRow}>
        {CATEGORIES.map(c => (
          <TouchableOpacity key={c} onPress={() => setCategory(c)}
            style={[s.catPill, category===c && { backgroundColor: theme.accent, borderColor: theme.accent }]}>
            <Text style={[s.catText, category===c && { color:"#fff" }]}>
              {CAT_ICONS[c]} {catLabel(c)}
            </Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
        ...
        {Object.entries(grouped).map(([cat, items]) => (
          <View key={cat}>
            ...
            {items.map(item => {
              ...
              return (
                <TouchableOpacity key={item.id} onPress={() => openAdvice(item)}
                  activeOpacity={0.8}
                  ...
                  <View style={[s.badge, { backgroundColor: STATUS_COLORS[status] }]}>
                    <Text style={s.badgeText}>
                      {status==="new"      ? (t.adviceNew       ?? "New")      :
                       status==="relevant" ? (t.adviceRelevant2 ?? "Relevant") :
          ...
          <View style={s.overlay}>
            <View style={[s.sheet, { backgroundColor: theme.bg }]}>
              <View style={[s.sheetHeader, { borderBottomColor: theme.border }]}>
                <Text style={[s.sheetTitle, { color: theme.text }]}>{modal.title}</Text>
                <TouchableOpacity onPress={() => setModal(null)} style={{ padding: 4 }}>
                  <Text style={{ color: theme.textMuted, fontSize: 22 }}>✕</Text>
                </TouchableOpacity>
              </View>
              <ScrollView style={{ padding: Spacing.lg }}>
                <Text style={[s.sheetBody, { color: theme.text }]}>{modal.body}</Text>
                <TouchableOpacity
                  onPress={() => toggleRelevant(modal.id)}
                  style={[s.relevantBtn, {
                    backgroundColor: relevant.has(modal.id) ? STATUS_COLORS.relevant : theme.surface,
                    borderColor: STATUS_COLORS.relevant,
                  }]}>
                  <Text style={[s.relevantBtnText, {
                    color: relevant.has(modal.id) ? "#fff" : STATUS_COLORS.relevant,
                  }]}>
...
const makeStyles = (t, insets) => StyleSheet.create({
  ...
  toggleRow:   { flexDirection:"row", margin:Spacing.md, backgroundColor:t.surface, borderRadius:Radius.full, borderWidth:1, borderColor:t.border, overflow:"hidden" },
  toggleBtn:   { flex:1, paddingVertical:10, alignItems:"center", borderRadius:Radius.full },
  toggleText:  { fontSize:FontSize.xs, fontWeight:"700", letterSpacing:1, color:t.textMuted },
  catRow:      { paddingHorizontal:Spacing.md, gap:8, paddingBottom:8, alignItems:"center" },
  catPill:     { paddingHorizontal:12, paddingVertical:6, borderRadius:Radius.full, borderWidth:1, borderColor:t.border, backgroundColor:t.surface },
  catText:     { fontSize:FontSize.xs, fontWeight:"600", color:t.textMuted },
  catHeader:   { fontSize:FontSize.lg, fontWeight:"800", marginTop:Spacing.lg, marginBottom:Spacing.sm, letterSpacing:0.3 },
  card:        { borderRadius:Radius.lg, borderWidth:1, padding:Spacing.md, marginBottom:Spacing.sm, shadowColor:"#000", shadowOpacity:0.05, shadowRadius:6, shadowOffset:{width:0,height:2}, elevation:2, position:"relative", overflow:"hidden" },
  badge:       { position:"absolute", top:0, right:0, paddingHorizontal:12, paddingVertical:5, borderBottomLeftRadius:Radius.md },
  badgeText:   { color:"#fff", fontSize:FontSize.xs, fontWeight:"700" },
  cardTitle:   { fontSize:FontSize.md, fontWeight:"600", marginTop:4, paddingRight:70, lineHeight:22 },
  cardRead:    { fontSize:FontSize.sm, fontWeight:"600", marginTop:Spacing.sm },
  empty:       { alignItems:"center", paddingVertical:60, gap:12 },
  emptyEmoji:  { fontSize:44 },
  emptyText:   { fontSize:FontSize.sm, textAlign:"center" },
  overlay:     { flex:1, backgroundColor:"rgba(0,0,0,0.5)", justifyContent:"flex-end" },
  sheet:       { borderTopLeftRadius:20, borderTopRightRadius:20, maxHeight:"80%", paddingBottom:40 },
  sheetHeader: { flexDirection:"row", alignItems:"flex-start", justifyContent:"space-between", padding:Spacing.lg, borderBottomWidth:1, gap:Spacing.md },
  sheetTitle:  { flex:1, fontSize:FontSize.lg, fontWeight:"700", lineHeight:26 },
  sheetBody:   { fontSize:FontSize.md, lineHeight:26, marginBottom:Spacing.xl },
  relevantBtn: { borderRadius:Radius.md, borderWidth:2, padding:Spacing.md, alignItems:"center", marginTop:Spacing.sm },
