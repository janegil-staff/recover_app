...
import {
  ...
  KeyboardAvoidingView, Platform, TextInput, Modal, FlatList, Image,
} from 'react-native';
...
function Dropdown({ label, value, options, onSelect, unit, theme }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <Text style={{ color: theme.textSecondary, fontSize: FontSize.sm, fontWeight: '600', marginBottom: 6 }}>
        {label}
      </Text>
      <TouchableOpacity
        ...
        activeOpacity={0.8}
      >
        <Text style={{ color: value ? theme.text : theme.textMuted, fontSize: FontSize.md, fontWeight: '600' }}>
          {value ? `${value}${unit ?? ''}` : '—'}
        </Text>
      ...
      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
        <TouchableOpacity style={s.modalOverlay} activeOpacity={1} onPress={() => setOpen(false)}>
          <View style={[s.modalSheet, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <TouchableOpacity
                    style={[s.modalItem, { borderBottomColor: theme.border }, active && { backgroundColor: theme.accent + '18' }]}
                    onPress={() => { onSelect(item); setOpen(false); }}
                    activeOpacity={0.7}
                  >
                    <Text style={{ color: active ? theme.accent : theme.text, fontSize: FontSize.md, fontWeight: active ? '700' : '500' }}>
                      {item}{unit ?? ''}
                    </Text>
      ...
      {[1, 2].map(n => (
        <React.Fragment key={n}>
          <View style={{
            ...
            borderWidth: 2, borderColor: step >= n ? theme.accent : theme.border,
            alignItems: 'center', justifyContent: 'center',
          }}>
            <Text style={{ fontSize: 12, fontWeight: '700', color: step >= n ? '#fff' : theme.textMuted }}>{n}</Text>
          </View>
          {n < 2 && <View style={{ width: 40, height: 2, backgroundColor: step >= 2 ? theme.accent : theme.border, marginHorizontal: 6 }} />}
        </React.Fragment>
      ))}
    </View>
  );
...
export default function RegisterScreen({ navigation, route }) {
  ...
  const canStep1 = tncAccepted && infoAccepted && pinSet && email.trim() && emailConfirm.trim();

  const goToPinSetup = () => navigation.navigate('PinSetup', {
    returnTo:     'Register',
    returnParams: { email, emailConfirm, language, tncAccepted, infoAccepted, gender, age, height, weight, step: 1 },
  });

  const advanceToStep2 = () => {
    setError('');
    const emailRx = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRx.test(email.trim()))                                          { setError(`${t.email} invalid`); return; }
    if (email.trim().toLowerCase() !== emailConfirm.trim().toLowerCase())     { setError(t.emailMismatch ?? 'Emails do not match'); return; }
    if (!pinSet)                                                               { setError(t.pinRequired ?? 'PIN required'); return; }
    if (!tncAccepted || !infoAccepted)                                         { setError(t.acceptTermsRequired ?? 'Please accept the terms'); return; }
    setStep(2);
  };

  const submit = async () => {
    setError('');
    if (!age) { setError(`${t.age} ${t.isRequired ?? 'is required'}`); return; }
    setLoading(true);
    try {
      await register({
        name:     email?.split('@')[0] ?? '',
        ...
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
          ...
          {step === 1 && <>
            ...
            <View style={{ marginBottom: Spacing.xl }}>
              <TouchableOpacity
                ...
                onPress={() => setLangOpen(o => !o)} activeOpacity={0.8}
              >
                <Text style={s.dropdownFlag}>{LANGUAGES.find(l => l.code === language)?.flag ?? '🌐'}</Text>
                <Text style={[s.dropdownValue, { color: theme.text }]}>{LANGUAGES.find(l => l.code === language)?.label ?? language}</Text>
                <Text style={[s.dropdownArrow, { color: theme.textMuted }]}>{langOpen ? '▲' : '▼'}</Text>
              </TouchableOpacity>
              {langOpen && (
                  ...
                  {LANGUAGES.map(({ code, flag, label }) => {
                    ...
                    return (
                      <TouchableOpacity key={code}
                        style={[s.dropdownItem, active && { backgroundColor: theme.accent + '14' }, { borderBottomColor: theme.border }]}
                        onPress={() => { setLanguage(code); setLang(code); setLangOpen(false); }} activeOpacity={0.7}
                      >
                        <Text style={s.dropdownFlag}>{flag}</Text>
                        <Text style={[s.dropdownItemLabel, { color: active ? theme.accent : theme.text }, active && { fontWeight: '700' }]}>{label}</Text>
                        {active && <Text style={{ color: theme.accent, fontSize: 16 }}>✓</Text>}
                      </TouchableOpacity>
                    );
                  })}
            ...
            <View style={s.checkRow}>
              <TouchableOpacity onPress={() => setTncAccepted(!tncAccepted)} activeOpacity={0.7}>
              ...
              <Text style={s.checkText}>
                {t.acceptTerms}{' '}
                <Text style={s.checkLink} onPress={() => navigation.navigate('Terms')}>{t.termsLink}</Text>
                {'\n'}{t.includingPrivacy}
              </Text>
            </View>
          ...
          {step === 2 && <>
              ...
              {[
                ...
                return (
                  <TouchableOpacity key={key} style={s.genderItem} onPress={() => setGender(key)} activeOpacity={0.7}>
                    <View style={[s.genderCircle, active ? { backgroundColor: theme.accent } : { backgroundColor: theme.accentLight ?? '#dde8f4' }]}>
                      <GenderIcon type={icon} active={active} color={active ? '#fff' : theme.accent} size={44} />
                    </View>
                    <Text style={[s.genderText, active && { color: theme.accent, fontWeight: '700' }]}>{label}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
            ...
            <View style={{ flexDirection: 'row', gap: Spacing.md, marginBottom: Spacing.xl }}>
              <Dropdown label={`${t.age}*`}                value={age}    options={AGES}    onSelect={setAge}    theme={theme} />
              <Dropdown label={t.heightCm ?? 'Height'}     value={height} options={HEIGHTS} onSelect={setHeight} unit=" cm" theme={theme} />
              <Dropdown label={t.weightKg ?? 'Weight'}     value={weight} options={WEIGHTS} onSelect={setWeight} unit=" kg" theme={theme} />
            </View>

            <TouchableOpacity style={[s.btn, (!age || loading) && s.btnDisabled]} onPress={age && !loading ? submit : undefined} activeOpacity={age ? 0.85 : 1}>
...
const makeStyles = (t) => StyleSheet.create({
  ...
  logoWrap: { width: 100, height: 100, borderRadius: 50, overflow: 'hidden', marginBottom: 8 },
  logo:     { width: 100, height: 100 },
  appName:  { color: t.text, fontSize: 22, fontWeight: '700', letterSpacing: 0.5, marginBottom: Spacing.lg },

  sectionLabel: { color: t.textSecondary, fontSize: FontSize.md, fontWeight: '700', marginBottom: Spacing.md, marginTop: Spacing.sm, letterSpacing: 0.3 },

  genderRow:    { flexDirection: 'row', justifyContent: 'space-between', marginBottom: Spacing.xl },
  genderItem:   { alignItems: 'center', flex: 1 },
  genderCircle: { width: 80, height: 80, borderRadius: 40, justifyContent: 'center', alignItems: 'center', marginBottom: Spacing.sm },
  ...
  pinRow:   { width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: Spacing.lg, paddingBottom: 8, borderBottomWidth: 2, borderBottomColor: t.inputLine },
  ...
  dropdown:      { flexDirection: 'row', alignItems: 'center', paddingHorizontal: Spacing.md, paddingVertical: 14, borderWidth: 1.5, borderColor: t.inputLine, borderRadius: Radius.md, backgroundColor: t.surface, gap: 10 },
  dropdownFlag:  { fontSize: 20 },
  dropdownValue: { flex: 1, fontSize: FontSize.md, fontWeight: '600' },
  dropdownArrow: { fontSize: 11 },
  dropdownList:  { borderWidth: 1.5, borderTopWidth: 0, borderBottomLeftRadius: Radius.md, borderBottomRightRadius: Radius.md, overflow: 'hidden', marginTop: -2 },
  dropdownItem:  { flexDirection: 'row', alignItems: 'center', paddingHorizontal: Spacing.md, paddingVertical: 12, borderBottomWidth: 1, gap: 10 },
  dropdownItemLabel: { flex: 1, fontSize: FontSize.md },

  // Inline dropdown trigger (for age/height/weight)
  dropTrigger: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 10, paddingVertical: 10, borderWidth: 1.5, borderColor: t.inputLine, borderRadius: Radius.sm, backgroundColor: t.surface },

  // Modal picker
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 32 },
  modalSheet:   { width: '100%', borderRadius: Radius.lg, borderWidth: 1.5, overflow: 'hidden' },
  modalTitle:   { fontSize: FontSize.md, fontWeight: '700', padding: Spacing.md, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.08)' },
  modalItem:    { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: Spacing.lg, paddingVertical: 12, borderBottomWidth: 1 },
  ...
  btn:         { width: '100%', height: 56, backgroundColor: t.accent, borderRadius: 10, justifyContent: 'center', alignItems: 'center', shadowColor: t.accent, shadowOpacity: 0.4, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: 6 },
  ...
  headerImage:  { width: '100%', height: 180, borderRadius: Radius.lg, marginBottom: Spacing.xl, overflow: 'hidden' },
});