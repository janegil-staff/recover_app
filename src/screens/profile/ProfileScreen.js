...
export default function ProfileScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    if (newPin)
      savePin(newPin).then(() =>
        Alert.alert(t.saved, t.pinUpdated ?? "PIN updated."),
      );
  }, []);
  ...
  const handleClearPin = () =>
    Alert.alert(
      t.signOutClearPin ?? "Sign out & clear PIN",
      t.clearPinMsg ?? "This will remove your PIN from this device.",
      [
        { text: t.cancel ?? "Cancel", style: "cancel" },
        {
          text: t.signOutClearPin ?? "Sign out & clear PIN",
          style: "destructive",
