...
function ToggleRow({ label, subtitle, value, onValueChange, theme, last, extra }) {
  return (
    <View style={{
      ...
      <View style={{ flex: 1 }}>
        <Text style={{ color: theme.text, fontSize: FontSize.md, fontWeight: '500',
          marginBottom: subtitle ? 3 : 0 }}>{label}</Text>
        {subtitle ? <Text style={{ color: theme.textSecondary, fontSize: FontSize.sm,
          lineHeight: 18 }}>{subtitle}</Text> : null}
        {extra ? <View style={{ marginTop: 6 }}>{extra}</View> : null}
      </View>
      <Switch value={value} onValueChange={onValueChange}
        trackColor={{ false: '#D1D5DB', true: '#22C55E' }}
        thumbColor="#fff" ios_backgroundColor="#D1D5DB" style={{ marginTop: 2 }} />
    </View>
  );
}
    ...
    <View style={s.root}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 40 }}
        ...
        <View style={s.section}>
          <ToggleRow label={t.activateReminder}
            subtitle={reminderEnabled ? 'Get a daily reminder to log' : 'Get a daily reminder to log'}
            value={reminderEnabled} onValueChange={toggle(setReminderEnabled, 'reminderEnabled')}
            theme={theme} />
          <ToggleRow label={t.darkMode}
            subtitle="Activate dark mode if you are sensitive to light."
            value={darkMode} onValueChange={handleDarkMode} theme={theme} />
          <ToggleRow label={t.requirePin}
            value={requirePin} onValueChange={toggle(setRequirePin, 'requirePin')}
            theme={theme} last />
        </View>
        ...
        {/* Tracking fields */}
        <View style={s.section}>
          <ToggleRow label={t.trackSubstances}
            subtitle="Log which substances were used each day."
            value={trackSubstances} onValueChange={toggle(setTrackSubstances, 'trackSubstances')}
            theme={theme} />
          <ToggleRow label={t.trackCravings}
            subtitle="Track craving intensity from 0 to 5."
            value={trackCravings} onValueChange={toggle(setTrackCravings, 'trackCravings')}
            theme={theme} />
          <ToggleRow label={t.trackMood}
            subtitle="Log mood and wellbeing each day."
            value={trackMood} onValueChange={toggle(setTrackMood, 'trackMood')}
            theme={theme} />
          <ToggleRow label={t.trackMedication}
            subtitle="Log whether you took your medication each day."
            value={trackMedication} onValueChange={toggle(setTrackMedication, 'trackMedication')}
            theme={theme} />
          <ToggleRow label={t.trackWeight}
            subtitle="Log your weight as part of your recovery journey."
            value={trackWeight} onValueChange={toggle(setTrackWeight, 'trackWeight')}
            theme={theme} last />
        </View>
        ...
        {/* Sharing */}
        <View style={s.section}>
          <ToggleRow label={t.weeklyReport}
            subtitle="Receive a weekly overview of your recovery trends."
            value={weeklyReport} onValueChange={toggle(setWeeklyReport, 'weeklyReport')}
            theme={theme} />
          <ToggleRow label={t.shareData}
            subtitle="Allow your clinician to view your logs and trends."
            value={shareWithDoctor} onValueChange={toggle(setShareWithDoctor, 'shareWithDoctor')}
            theme={theme} last />
        </View>
